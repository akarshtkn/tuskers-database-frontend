...
const Select = React.memo(({options, field, selectValue, selectfn, error}:Props) => {
  ...
  return (
    ...
    {field && <div className=" flex text-zinc-50 text-xl">{field}</div>}
    <Listbox value={selectValue} onChange={handleChange}>
      {({ open }) => (
        <>
          <div className="relative">
            <ListboxButton className="relative w-full cursor-default rounded-md bg-zinc-100 h-9 py-1.5 pl-1 pr-10 text-left shadow-sm ring-1 ring-inset ring-amber-300 focus:outline-none focus:ring-2 focus:ring-amber-500 sm:text-sm sm:leading-6">
